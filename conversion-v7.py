...
- Builds an index of internal labels and converts:
    :ref:`text<label>` and {ref}`text<label>` -> [text](/docs/...#label)
    :ref:`label` / {ref}`label`              -> [label](/docs/...#label)
...
- Fixes a specific broken pattern where a second MyST image block
  got merged into:

    ![...](recovery.png){image} path
    :alt: ...
    :width: ...
    ```

  and normalizes *all* image paths to start with /../images/… (or similar).
"""
...
def normalize_image_path(path: str) -> str:
    """
    Normalize image paths so they always look like:
      /../images/...

    - If the path is already absolute (starts with '/'), we leave it as is.
    - If it starts with ../ or ../../ etc., we strip that and prefix /../
    """
    path = path.strip()
    if path.startswith("/"):
        return path
    ...
    stripped = re.sub(r"^(\.\./)+", "", path)  # "../images/foo.png" -> "images/foo.png"
    ...
    while i < len(lines):
        ...
        if m:
            ...
            if mimg:
                ...
                norm = normalize_image_path(path)
                var_map[key] = f"![{alt}]({norm})"
...
def build_label_index(docs_root: Path) -> Dict[str, str]:
    ...
    print(f"[info] Indexed {len(index)} internal labels for :ref: / {{ref}}")
    return index
...
def convert_ref_roles(text: str, label_index: Dict[str, str]) -> str:
    """
    Convert:
      :ref:`text<label>` / {ref}`text<label>` → [text](resolved_url)
      :ref:`label` / {ref}`label`             → [label](resolved_url) when possible.
...
/>
    ...
    # 1) Convert ::: {note} ... ::: blocks to simple <Callout variant="...">body</Callout>
    ...
    text = colon_pattern.sub(repl_colon, text)

    # 2) Convert ```{note} ... ``` fenced blocks to simple Callout
    fenced_pattern = re.compile(
        r"```{(note|warning|tip|important|caution)}[^\n]*\n([\s\S]*?)\n```",
        re.IGNORECASE,
    )

    def repl_fenced(m: Match[str]) -> str:
        kind = m.group(1).lower()
        body = m.group(2).strip("\n")
        variant = ADMONITION_MAP.get(kind, "info")
        return f"<Callout variant=\"{variant}\">\n\n{body}\n\n</Callout>"

    text = fenced_pattern.sub(repl_fenced, text)

    # 3) Upgrade all <Callout variant="...">body</Callout> to Tina's body={<></>} format
    upgrade_pattern = re.compile(
        r"<Callout variant=\"([^\"]+)\">\s*\n([\s\S]*?)\n</Callout>",
        re.MULTILINE,
    )

    def upgrade_repl(m: Match[str]) -> str:
        variant = m.group(1)
        body = m.group(2).strip("\n")
        # We don't try to re-indent aggressively; just wrap the body.
        return (
            "<Callout\n"
            "  body={<>\n"
            f"{body}\n"
    ...
    text = upgrade_pattern.sub(upgrade_repl, text)
...
def convert_rst_images(lines: List[str]) -> List[str]:
    ...
    to Markdown images:
      ...
      ![text](path)  (normalized to /../images/...)
    """
    out: List[str] = []
    i = 0
    while i < len(lines):
...
def convert_myst_image_blocks(text: str) -> str:
    ...
    def repl(m: Match[str]) -> str:
        ...
        alt = malt.group(1).strip() if malt else ""
        norm = normalize_image_path(path)
        return f"{indent}![{alt}]({norm})"

    return pattern.sub(repl, text)


def fix_broken_myst_image_leftovers(text: str) -> str:
    """
    Fix cases where a second MyST image got mangled into:

      ![...](recovery.png){image} ../images/.../create_account.png
      :alt: some alt
      :width: 50%
      ```
    ...
    and turn that into:
      ...
      ![some alt](../images/.../create_account.png)

    (the final normalization pass will fix the path format)
    """
    pattern = re.compile(
        r"\)\{image\}\s+([^\s]+)\s*\n\s*:alt:\s+(.*)\n[\s\S]*?```",
        re.MULTILINE,
    ...
    def repl(m: Match[str]) -> str:
        ...
        alt = m.group(2).strip()
        # keep the preceding ')' from the first image, then add a new image
        return f")\n\n![{alt}]({path})"

    return pattern.sub(repl, text)


# ----------------- NORMALISE ALL MARKDOWN IMAGES ----------------- #

def normalize_all_markdown_images(text: str) -> str:
    """
    Post-process all Markdown images:

      ![alt](path)

    and normalize their paths using normalize_image_path.
    """
...
def convert_text(text: str, var_map: Dict[str, str], label_index: Dict[str, str]) -> str:
    ...
    # 4. Resolve :ref: / {ref} roles using the label index
    ...
    # 8. Fix broken create_account-style leftovers
    text = fix_broken_myst_image_leftovers(text)

    # 9. Convert admonitions (:::{note} etc. + ```{note}```) → Callout with body={<></>}
    text = convert_admonitions(text)

    # 10. Normalize all markdown image paths to /../images/...
    text = normalize_all_markdown_images(text)
