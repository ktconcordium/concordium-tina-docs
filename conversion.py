...
- Converts ```{eval-rst} .. dropdown:: title ... ``` blocks into:
...
def load_variables(variables_path: Path) -> Dict[str, str]:
    ...
    if not variables_path.exists():
        print(f"[warn] variables.rst not found at {variables_path}, skipping variable substitution")
        return mapping
...
def convert_ref_roles(text: str, label_index: Dict[str, str]) -> str:
    ...
    If label cannot be resolved, the plain text is left.
    """
    ...
    def repl(m: Match[str]) -> str:
        ...
        # Fallback: keep visible text only
        return text_part
...
def convert_eval_rst_block(inner: str) -> str:
    ...
    return f'<Accordion heading="{safe_heading}">\n\n{content}\n\n</Accordion>'
...
def convert_text(text: str, var_map: Dict[str, str], label_index: Dict[str, str]) -> str:
    ...
    # 6. Convert eval-rst dropdowns -> Accordion
    text = convert_eval_rst_blocks(text)

    # 7. Convert admonitions (:::{note} etc.)
    text = convert_admonitions(text)

    return text


def process_file(path: Path, var_map: Dict[str, str], label_index: Dict[str, str]):
    original = path.read_text(encoding="utf-8")
    converted = convert_text(original, var_map, label_index)
...
def main():
    if len(sys.argv) < 3:
        ...
        print("Example: python convert_sphinx_to_tina.py content/docs source/variables.rst")
        sys.exit(1)
