...
Add one final change to the `tokenMetadata()` function. As you can see in [this previous tutorial](/docs/mainnet/tutorials/nft-minting/mint-xfer#nft-view-fn), this function combines the `url` value and `token_id` and returns it. Instead, you are going to read the `url` from the `state` with `token_id` and return it.
...
![](/images/build-contract-module.png)console
...
![](/images/contract-deploy-success-ccdscan.png)console
...
![](/images/contract-initialize-success.png)