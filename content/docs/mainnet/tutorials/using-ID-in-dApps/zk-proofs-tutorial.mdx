...
The ZK statements use Concordium's [AtomicStatementV2](https://docs.concordium.com/concordium-node-sdk-js/types/web3_id.AtomicStatementV2.html) type to define:
...
<Callout
  body={<>
Country codes to use for residence and nationality proofs are the [ISO-3166-1 alpha-2](https://www.iso.org/iso-3166-country-codes.html) codes.
  </>}
  variant="info"
/>
...
- **Security purpose**: Including a recent block hash in the challenge creates time-limited ZK proofs. After the chain progresses further, the proofs will no longer be valid, preventing [replay attacks](https://csrc.nist.gov/glossary/term/replay_attack).
...
- **Blockchain interaction**: The function uses [ConcordiumGRPCClient](https://docs.concordium.com/concordium-node-sdk-js/classes/grpc.ConcordiumGRPCClient.html) to communicate with a Concordium node:
...
- A connection to the Concordium node using [ConcordiumGRPCClient](https://docs.concordium.com/concordium-node-sdk-js/classes/grpc.ConcordiumGRPCClient.html) with `GrpcWebFetchTransport`
...
The recent block is important for security - it ensures the proof will expire after the chain progresses further, preventing [replay attacks](https://csrc.nist.gov/glossary/term/replay_attack).
...
Proper implementation would send the presentation to a backend [verifier](https://github.com/Concordium/concordium-web3id/tree/main/services/web3id-verifier).
  </>}
  variant="warning"
/>
...
You can explore this [github repository](https://github.com/Concordium/concordium-web3id/tree/main/services/web3id-verifier) which contains a verifier.
...
1. Define what should be proven
2. Generate a challenge
3. Process the returned proof
