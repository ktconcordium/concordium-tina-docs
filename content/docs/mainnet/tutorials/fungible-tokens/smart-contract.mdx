...
When you get the parameters, ensure the token exists with the `ensure!` and `contains_token()` functions. Note that, when you call the `burn()` function, you need to emit the `BurnEvent`. For more details, see the [CIS-2] standard documentation.
...
First, create a metadata file. You will use it to pick an image for it, and naming and description. One thing that is important to note is the **unique** flag. Since this is not an NFT, you should set it to **false**. Then upload it to the IPFS and save the CID address. For information about how to do this, see [Upload the NFT](/docs/mainnet/tutorials/nft-minting/upload-nft#upload-nft). You will use it while minting the token.
...
![dark console screen with result of running the command above](/../images/build-sc.png)console
...
![dark console screen with result of running the command above](/../images/deploy-sc.png)console
...
![dark console screen with result of running the command above](/../images/initialize-sc.png)

[Click here](/docs/mainnet/tutorials/fungible-tokens/mint-xfer#mint-xfer-ft) to continue to the second part of the tutorial.

[cis-2]: https://proposals.concordium.software/CIS/cis-2.html
[ft-smart-contract]: https://proposals.concordium.software/CIS/cis-2.html
