---
title: "Writing the piggy bank smart contract"
---

(piggy-bank-writing)=


This is the first [part of a tutorial](/docs/mainnet/tutorials/piggy-bank/index#piggy-bank) on smart contract
development. In this part you will focus on how to write a smart contract in the
[Rust] programming language using the {{ concordium-std }} library.

## Preparation

Set up a new smart contract project by following the guide [setup-contract](/docs/mainnet/docs/smart-contracts/guides/setup-contract#setup-contract) and return to this point afterwards.
...
In `lib.rs`, start by bringing everything from the [concordium-std] library into scope
by adding the line:
...
When using the [concordium-std] library, this all boils down to your type
for the contract state having to implement the [Serialize] [^s] trait exposed by
[concordium-std].

Luckily the library already contains implementations for most of the primitives
and standard types in [Rust], and a [procedural macro for deriving]
[Serialize] for most cases of enums and structs:
...
[^s]: In certain cases, the `Serial` and `DeserialWithState`
    ...
    [serialize-state-and-parameters](/docs/mainnet/docs/smart-contracts/develop-contracts#serialize-state-and-parameters) for more information.
...
<Callout
  body={<>
A [Rust] developer could compare init functions with the convention of
...
In [Rust] an init function can be specified as a regular function that is annotated
with a procedural macro from {{ concordium-std }} called [init].
...
`"PiggyBank"`. The name [^valid-name] is used as part of the exported
...
<Callout
  body={<>
For a [Rust] developer, receive functions are like methods with
...
[receive], which, like [init], is used to annotate a function and sets up an
external function and supplies you with an interface for accessing the context.
But, unlike the [init] macro, the function for [receive] is supplied with
...
This name should match the name in the corresponding attribute in [init]
...
Make sure that the contract name matches the one you use for the [init] macro,
...
[Rust] in general, {{ concordium-std }} exposes a [bail] macro:
...
(smashing-the-piggy-bank-writing)=
...
To compare the `sender` with `owner` you can use the [matches_account]
...
<Callout
  body={<>
To view return values of a contract instance on the chain, see the guide [invoke-instance](/docs/mainnet/docs/smart-contracts/guides/invoke-instance#invoke-instance).
  </>}
  variant="info"
/>
...
To continue with the tutorial click [here](/docs/mainnet/tutorials/piggy-bank/testing#piggy-bank-testing).

[bail]: https://docs.rs/concordium-std/latest/concordium_std/macro.bail.html
[concordium-std]: https://docs.rs/concordium-std/latest/concordium_std/index.html
[ensure]: https://docs.rs/concordium-std/latest/concordium_std/macro.ensure.html
[here]: https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/examples/piggy-bank/part1/src/lib.rs
[init]: https://docs.rs/concordium-std/latest/concordium_std/attr.init.html
[invoke_transfer]: https://docs.rs/concordium-std/latest/concordium_std/struct.ExternHost.html#tymethod.invoke_transfer
[matches_account]: https://docs.rs/concordium-std/latest/concordium_std/enum.Address.html#method.matches_account
...
[procedural macro for deriving]: https://docs.rs/concordium-std/latest/concordium_std/derive.Serialize.html
[receive]: https://docs.rs/concordium-std/latest/concordium_std/attr.receive.html
[rust]: https://www.rust-lang.org/
[self_balance]: https://docs.rs/concordium-std/latest/concordium_std/struct.ExternHost.html#tymethod.self_balance
[serialize]: https://docs.rs/concordium-std/latest/concordium_std/trait.Serialize.html