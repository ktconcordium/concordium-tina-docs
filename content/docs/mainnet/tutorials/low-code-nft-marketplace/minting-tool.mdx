---
title: "Low-code NFT minting tool"
---
...
- Connect with your Concordium Wallet for Web
...
- [Yarn](https://classic.yarnpkg.com/en/) - a performance package manager for JavaScript; replaces the `npm` client.
...
2. Now run your store web server locally with the following command.
...
![minting tool front page](/../images/minting-tool.png)

## Mint

Concordium provides a [library of sample images you can download](https://github.com/bogacyigitbasi/sample-images/blob/main/NFTs.zip) and use to practice minting NFTs.

<accordion
  heading="Mint NFTs with metadata"
  docText={<>
...
1. When your platform is up and running, click **DEPLOY NEW** to mint your brand-new NFTs as shown below.

![minting tool screen with deploy new button](/../images/minting-tool-deploy-new.png)
2. The wallet pops up and asks for your signature. Click **Sign & Submit**.

![minting tool screen with wallet screen pop up](/../images/minting-tool-wallet-sign.png)
3. You will see the figure below, since you are going to provide the metadata.json file, you don't need to use the connect Pinata function. Click **SKIP**.

![minting tool screen with connect to pinata shown](/../images/minting-tool-skip-pinata.png)
...
When every step is completed as described, the platform will show you the preview of the token and you can check even the metadata by clicking the token's display. If all is ok, click **MINT**. Your Concordium Wallet for Web will pop up once more and ask for your approval. You can check the token details in the wallet, minting fees, etc.
...
.. Note::
    ...
    Pinata is a commercial, pinning solution that pins your data stored on IPFS. You can find more info about it [in this link](https://app.pinata.cloud/).
...
![pinata screen shown](/../images/pinata.png)
**NFT minting**

Now you will create a brand new contract instance for your NFT collection.

1. Click **MINT** in the top navigation bar of your marketplace.

2. In the Connect Pinata screen paste your API key and click **CONNECT**.

![nft marketplace connection screen after clicking mint](/../images/connect-pinata.png)
3. Once connected, upload images either by selecting them from a folder or just dragging and dropping them. You are not limited to uploading 1-2 images, and thanks to the template, the metadata generator is easier than creating for one metadata file for every item.

![screen showing multiple image selection](/../images/upload-images.png)
 **Mint NFT collection**

4. Set your token IDs and click **Upload** for each.

![images uploaded and need id before creation](/../images/upload-set-token-id.png)
...
![images after data fetched](/../images/fetch-pinata-metadata.png)
5. Add data to them and the marketplace will create metadata for you when you click **CREATE** for each of the tokens. Specify the quantity as **1** and click **DONE**.

![editing metadata and create option for each image](/../images/edit-metadata-create.png)
![editing metadata](/../images/metadata-nearly-complete.png)
6. Click **MINT**. Your Concordium Wallet for Web prompts you to click **Sign and Submit** to approve the transaction and mint your NFTs.

![mint option shown in background dimmed with wallet popup visible](/../images/mint-w-wallet-popup.png)
  </>}
  image=""
  fullWidth={true}
/>
...
![prepare to mint tokens from an existing contract by entering contract address](/../images/mint-from-contract-index.png)
It will check if the contract has some unique CIS-2 methods, like the ``supports()`` function which means the contract is compliant with CIS-2. Basically, you are checking whether the contract is a token contract or not. If you are the owner, you will be able to mint your new tokens exactly [as in the previous sections](/docs/mainnet/tutorials/low-code-nft-marketplace/marketplace#mint-w-metadata). If you are not the owner of the contract, then you will end up with the error shown below.

![error shown if you are not the owner of the contract you are executing](/../images/mint-error.png)
  </>}
  image=""
  fullWidth={true}
/>
