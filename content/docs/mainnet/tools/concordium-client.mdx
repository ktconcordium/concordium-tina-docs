...
through a [local node](/docs/mainnet/docs/network/nodes/run-node-docker/run-node-docker#run-a-node) on `127.0.0.1` and port `20000` for mainnet. If the node runs on a different machine or in a custom setup, the options `--grpc-ip` and `--grpc-port` can be used
to set the [IP address] and [port number] where the node is accessible. These flags are supported by all `concordium-client` commands. Note that as of version
...
[queries](/docs/mainnet/tools/query-node#testnet-query-node) and others send [transactions](/docs/mainnet/tools/transactions#transactions-old).
  </>}
  variant="info"
/>
...
<Callout
  body={<>
All transfers and transactions cost a fee. The fee is based on the set [NRG](/docs/mainnet/docs/protocol/transaction-fees#transaction-fees) for that transaction and the current exchange rate.
...
<Callout
  body={<>
`concordium-client` binary is available on the [Downloads](/docs/mainnet/docs/installation/downloads#downloads) page
  </>}
  variant="info"
/>
...
- decryption key-pair used for decrypting the shielded balance ([deprecated](/docs/mainnet/docs/guides/shield-ccd-wallets#shielded-balance-feature-deprecation)) of the
  account.
...
Read more about accounts [here](/docs/mainnet/docs/protocol/manage-accounts#managing_accounts).
...
section) and the stored key for decrypting its shielded balance ([deprecated](/docs/mainnet/docs/guides/shield-ccd-wallets#shielded-balance-feature-deprecation)) (under the
...
See also the [Managing accounts](/docs/mainnet/docs/protocol/manage-accounts#managing_accounts) section.
...
(concordium-client-import-accounts-keys)=
...
##### Importing accounts from Concordium Wallet for Web or Concordium Wallet

You can export a single account as a JSON file from the Concordium Wallet for Web or Concordium Wallet.
...
##### Importing accounts from Concordium Legacy Wallet

The Concordium Legacy Wallet allows you to export all accounts at once using the **Backup** option.
...
<Callout
  body={<>
The Concordium Legacy Wallet is deprecated, does not receive updates and is not recommended for use.
  </>}
  variant="info"
/>
...
A detailed description and explanation of the term alias is available here [Account aliases](/docs/mainnet/tools/transactions#account-aliases)
...
[framework]: https://github.com/pcapriotti/optparse-applicative#user-content-bash-zsh-and-fish-completions
