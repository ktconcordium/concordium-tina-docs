---
title: "CCDScan Staking overview"
---
...
![dark screen with graphs at top, table at bottom showing validator information](/../images/ccd-scan-bakers.png)

![dark screen with block details](/../images/ccd-scan-staking-payday-block-details.png)

The Pay Day block details contains the following:

- **Block**: the first six characters of the block hash. Click Copy ![Green document on top of another green document](/../images/ccd-scan-copy.png) to copy the entire hash. The block status is also shown.
- **Age**: the age of the block and date/time stamp when it was created.
- **Validator id**: ID of the validator who produced the block. Click the validator ID to see the [validator details](/docs/mainnet/tools/ccd-scan/ccd-scan-staking#home-screen-baker).
- **Tokenomics**: contains the following details about the block:
  : - Distributed minted CCD: Contains the total minted CCDs and a breakdown on their distribution across the block reward account, finalization reward account, and foundation account. Note that finalization rewards are zero as of December 7, 2023.
    - Accrued block rewards: similar to a ‘regular’ [block](/docs/mainnet/tools/ccd-scan/ccd-scan-blocks#home-screen-block); contains the accrued block rewards.
    - Payday foundation rewards: displays the payout to the foundation account for the pay day period and contains the option to drill-through to the [foundation account](/docs/mainnet/tools/ccd-scan/ccd-scan-accounts#accounts-view).
...
- **Validator ID**: ID of the validator. This is the ID you need if you want to delegate to the staking pool in a wallet. Click the validator ID to see the [validator details](/docs/mainnet/tools/ccd-scan/ccd-scan-staking#home-screen-baker). If the validator has been removed (i.e. the validator is no longer active), **Removed** appears next to the ID.
...
- **Available for delegation**: shows the maximum number of remaining CCDs available for delegation based on the capital bound and the leverage factor between a validator and associated delegators. Based on the [cap](/docs/mainnet/docs/protocol/concepts-delegation#delegation-caps) imposed.
...
![dark screen showing table of information about top delegators](/../images/ccd-scan-staking-delegators.png)

![dark screen with details about passive delegation](/../images/ccd-scan-passive-delegation.png)
...
- **Rewards**: list of rewards paid out during the selected time range. Includes an overview graph that shows the aggregated rewards for passive delegation based on the applied filter. Rewards details are displayed in a table.
  : - **Time** shows the date/time the reward was paid
    - **Reference** shows the first six characters of the block hash in which the reward was paid. You can click Copy ![Green document on top of another green document](/../images/ccd-scan-copy.png) to copy the entire hash. Click the hash to see the [block details](/docs/mainnet/tools/ccd-scan/ccd-scan-blocks#home-screen-block) for the block where the award payout occurred.
    - **Rewards** shows the amount of reward distributed.
- **Delegators**: the delegators staking to passive delegation, their restake preference, and amount staked. Click on a **Hash** to see [account details](/docs/mainnet/tools/ccd-scan/ccd-scan-accounts#accounts-view). You can click Copy ![Green document on top of another green document](/../images/ccd-scan-copy.png) to copy the entire account address.
...
![dark screen showing details of a single validator](/../images/ccd-scan-baker-details.png)
...
- **Payday rewards**: shows the sum of rewards earned by the validator and the individual rewards transactions for the time range selected in the filter.
  : - **Time**: shows the date/time the reward was distributed.
    - **Reference**: shows the first six characters of the transaction hash. Click on the first six characters of a hash in Reference to see [information about the transaction](/docs/mainnet/tools/ccd-scan/ccd-scan-transactions#home-screen-transaction). Click Copy ![Green document on top of another green document](/../images/ccd-scan-copy.png) to copy the entire hash number.
