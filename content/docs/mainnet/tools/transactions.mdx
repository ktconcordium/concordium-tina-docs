---
title: "Concordium Client transactions"
---

(transactions-old)=


You can perform all types of transactions with the [concordium-client](/docs/mainnet/tools/concordium-client#concordium-client). To do so, you use specialized subcommands.
...
<Callout
  body={<>
All transfers and transactions cost a fee. The fee is based on the set [NRG](/docs/mainnet/docs/protocol/transaction-fees#computing-transaction-costs) for that transaction and the current exchange rate.
...
| `transaction send-scheduled`      | Make a transfer that will be released gradually                                                    |
| `validator add`                   | Add a new validator. For more information, see [become-a-validator](/docs/mainnet/docs/network/guides/become-validator#become-a-validator).                          |
| `validator remove`                | Remove a validator. For more information, see [become-a-validator](/docs/mainnet/docs/network/guides/become-validator#become-a-validator).                           |
| `validator update-stake`          | Update the staked amount of a validator. For more information, see [become-a-validator](/docs/mainnet/docs/network/guides/become-validator#become-a-validator).      |
| `validator update-restake`        | Update the restaking switch of a validator. For more information, see [become-a-validator](/docs/mainnet/docs/network/guides/become-validator#become-a-validator).   |
| `validator set-key`               | Update the keys of a validator. For more information, see [become-a-validator](/docs/mainnet/docs/network/guides/become-validator#become-a-validator).               |
| `account update-keys`             | Update credentials keys for a specific credential                                                  |
| `account unshield`                | Transfer part of the shielded balance to the public balance                                        |
| `account show`                    | Show account information. [See below for specific information](/docs/mainnet/tools/transactions#account-commands).             |
| `identity show`                   | Show identity information. [See below for specific information](/docs/mainnet/tools/transactions#identity-commands).           |
| `delegator configure`             | Add, configure, and remove delegation. [See below for information](/docs/mainnet/tools/transactions#delegation-commands).      |
| `consensus show-chain-parameters` | Show chain parameters. [See below for specific information](#consensus show-chain-parameters). |
...
- `--sender`: Name or address of the transaction's sender account.
  The name is the one that's used when you [import the account](/docs/mainnet/docs/protocol/manage-accounts#managing_accounts) (assuming that this
...
Each account on the Concordium blockchain has a [sequence number](/docs/mainnet/docs/resources/glossary#transaction-sequence-number) and each
...
The Concordium Wallet for Mobile and Concordium Legacy Wallet keep track of the sequence number and assign the correct one when sending transactions.
...
matching account [aliases](/docs/mainnet/docs/resources/glossary#alias). The canonical account address is derived when an account is created on chain. The other 16 million addresses with matching initial 29 bytes are referred to as account aliases for
...
The add, remove, and configure validators commands are described in the topic [becoming a validator using the Concordium Client](/docs/mainnet/docs/network/guides/become-validator#become-a-validator).
...
The following flags are for adding a [transfer memo](/docs/mainnet/docs/resources/glossary#transfer-memo) to the transfer.
