---
title: "FAQ for Ethereum developers"
---
...
<accordion
  heading="Where can I find a smart contract reference library similar to the GitHub repo from `OpenZeppelin`?"
  docText={<>
...
<accordion
  heading="How are `smart contract addresses` represented on Concordium?"
  docText={<>
...
<accordion
  heading="How are `externally owned addresses` represented on Concordium?"
  docText={<>
In terms of naming, Concordium uses `contract` and `account` to refer
to the Ethereum equivalent of a `smart contract` and an `externally owned address`, respectively.
The word `address` refers to either an `account` address or a `contract` address on Concordium.
...
<accordion
  heading="What is the equivalent to `msg.sender` and `tx.origin` on Concordium?"
  docText={<>
...
<accordion
  heading="Can I force CCD to a smart contract even if it has no payable function?"
  docText={<>
...
<accordion
  heading="How do I embed a schema into a smart contract? Why do I need a schema? How can I provide the input parameters as a JSON object and get the output parameters in a human-readable format when using the `concordium-client`?"
  docText={<>
...
<accordion
  heading="Is there something similar to the ERC20 standard?"
  docText={<>
...
<accordion
  heading="Is there something similar to the ERC721 standard?"
  docText={<>
Yes, please read the [CIS-2 standard](https://proposals.concordium.software/CIS/cis-2.html).
The `CIS-2` standard can represent fungible and non-fungible tokens.
...
<accordion
  heading="Is there something similar to the ERC165 standard?"
  docText={<>
Yes, please read the [CIS-0 standard](https://proposals.concordium.software/CIS/cis-0.html).
Please explore the [CIS-2 library](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/concordium-cis2/src/lib.rs)
...
<accordion
  heading="Is there something similar to a wrapped token contract?"
  docText={<>
Yes, please explore:
...
<accordion
  heading="Does Concordium have something similar to `delegateCall`?"
  docText={<>
No. A contract on Concordium can only change its own state. If you are looking for an upgradable pattern, please explore
the  [upgradable wCCD implementation](https://github.com/Concordium/concordium-rust-smart-contracts/pull/128).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Can I create a factory smart contract on Concordium?"
  docText={<>
...
The guide to [using the factory pattern on Concordium](/docs/mainnet/docs/smart-contracts/guides/factory-pattern#factory-pattern) discusses
...
<accordion
  heading="Can I predict/calculate the address of the smart contract before deploying it? Is there something similar to the Ethereum CREATE2?"
  docText={<>
...
<accordion
  heading="Is there something similiar to gas and transaction fees?"
  docText={<>
