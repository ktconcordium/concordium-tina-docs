---
title: "FAQ for Solana developers"
---
...
Feel free to participate and add your questions to the [Concordium software support channels](https://support.concordium.software/).

## Concordium smart contracts

<accordion
  heading="What are `Solana programs` called on Concordium?"
  docText={<>
Programs are called smart contracts on Concordium.
See the comparison [here](/docs/mainnet/docs/help-and-faq/onboarding-guide-solana-developers/overview#solana-programs).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="What smart contract language is used on Concordium?"
  docText={<>
...
<accordion
  heading="How can I start with the Rust smart contract language?"
  docText={<>
...
[Concordium Rust smart contract repo](https://github.com/Concordium/concordium-rust-smart-contracts/tree/main/examples).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Where can I find example smart contracts?"
  docText={<>
You can find examples of smart contracts in the
[Concordium Rust smart contract repo](https://github.com/Concordium/concordium-rust-smart-contracts/tree/main/examples).
  </>}
...
<accordion
  heading="Where can I find a smart contract reference library similar to the GitHub repo from `Solana Program Library`?"
  docText={<>
...
<accordion
  heading="Should I use V0 or V1 smart contracts?"
  docText={<>
...
<accordion
  heading="How can I write a smart contract?"
  docText={<>
You can follow the chapter [writing a smart contract](/docs/mainnet/tutorials/piggy-bank/writing#piggy-bank-writing) in the piggy bank tutorial.
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="How can I test a smart contract?"
  docText={<>
You can follow the chapter [testing a smart contract](/docs/mainnet/tutorials/piggy-bank/testing#piggy-bank-testing) in the piggy bank tutorial.
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Is there something similar to Solana's `Anchor` framework for Concordium?"
  docText={<>
Yes. The Concordium standard library crate [concordium-std](https://crates.io/crates/concordium-std) provides a high-level interface for writing smart contracts in Rust.
...
Together with [cargo-concordium](/docs/mainnet/docs/smart-contracts/guides/build-contract#build-contract) it can be used to develop, test and build smart contracts.

You can follow the chapter [writing a smart contract](/docs/mainnet/tutorials/piggy-bank/writing#piggy-bank-writing) in the piggy bank tutorial to get started with using [concordium-std](https://crates.io/crates/concordium-std).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="How can I update account data?"
  docText={<>
...
See [solana-overview](/docs/mainnet/docs/help-and-faq/onboarding-guide-solana-developers/overview#solana-overview) for the details.
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Is there something similar to native programs in Concordium?"
  docText={<>
...
The list of supported host functions is available in [host-functions](/docs/mainnet/docs/smart-contracts/references/host-fns#host-functions).
...
<accordion
  heading="Can I build tests that print a table of content structure (a layered output similar to mocha and chai tests)?"
  docText={<>
...
<accordion
  heading="How are `program addresses` represented on Concordium?"
  docText={<>
...
See the [overview of differences](/docs/mainnet/docs/help-and-faq/onboarding-guide-solana-developers/overview#solana-overview) for more details.
...
<accordion
  heading="How are `wallet addresses` represented on Concordium?"
  docText={<>
In terms of naming, Concordium uses *contract* and *account* to refer
to a similar Solana's *executable account* (programs) and *non-executable account*, respectively.
Note, however that the correspondense is not precise.
...
<accordion
  heading="Can I distinguish between contract and account addresses?"
  docText={<>
Yes. You can distinguish between the different types of addresses
in smart contracts on the Concordium chain.
[Addresses](https://docs.rs/concordium-std/latest/concordium_std/enum.Address.html)
...
is executed. You can read more about [pattern matching](https://doc.rust-lang.org/book/ch18-03-pattern-syntax.html) in the
Rust language book.
...
<accordion
  heading="How do I get the transaction sender address on Concordium?"
  docText={<>
...
<accordion
  heading="How do I get the balance of the smart contract within the Rust code?"
  docText={<>
...
Additional documentation can be found in the [self_balance description](https://docs.rs/concordium-std/latest/concordium_std/struct.ExternHost.html#tymethod.self_balance)
...
<accordion
  heading="How do I get the address of the smart contract within the Rust code?"
  docText={<>
``ctx.self_address()`` returns the address of the smart contract.
Additional documentation can be found in the [self_address description](https://docs.rs/concordium-std/latest/concordium_std/struct.ExternContext.html#tymethod.self_address) of the concordium standard crate.
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Can I print values from the smart contract code or test cases?"
  docText={<>
Yes. You can use
...
<accordion
  heading="My transaction is rejected and I get an error code number. How can I interpret smart contract errors?"
  docText={<>
...
```
![](../../../tutorials/piggy-bank/images/pb_tutorial_21.png)
You can find the meaning of common error codes in this [list](https://docs.rs/concordium-std/latest/concordium_std/#signalling-errors).
...
```
![](../../../tutorials/piggy-bank/images/pb_tutorial_27.png)
...
<accordion
  heading="Is there a smart contract code linter?"
  docText={<>
Yes. You can use the ``fmt`` and the ``cargo clippy`` linter tools as described in the [README](https://github.com/Concordium/concordium-rust-smart-contracts).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Are tests executed in parallel or sequentially?"
  docText={<>
...
<accordion
  heading="How can I provide the input parameters as a JSON object and get the output parameters in a human-readable format when using the `concordium-client`? What is a smart contract schema?"
  docText={<>
A [smart contract schema](/docs/mainnet/docs/smart-contracts/contract-schema#contract-schema) is a description of how to represent
...
```
![](./images/onboarding_ethereum_developers_5.png)
...
```
![](./images/onboarding_ethereum_developers_6.png)
...
![](./images/onboarding_ethereum_developers_7.png)
...
![](./images/onboarding_ethereum_developers_8.png)
...
![](./images/onboarding_ethereum_developers_9.png)
...
![](./images/onboarding_ethereum_developers_10.png)
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="How can I create the parameter data if I want to use binary input instead of a JSON input?"
  docText={<>
When using the ``concordium-client`` to interact with smart contracts the input and output parameters
can be either in human-readable format (with a schema) or in raw bytes (binary format).
...
<accordion
  heading="Where can I find a logged event on testnet/mainnet?"
  docText={<>
You can look up a transaction hash on the dashboard such as [this example](https://dashboard.testnet.concordium.com/lookup/13ded9aaf6085e970b2cf3874431de5805ffa35a553c93707d1863a8888e8aa4).
...
For example, navigate to the last page (third page) of the displayed execution chain of [this transaction](https://dashboard.testnet.concordium.com/lookup/13ded9aaf6085e970b2cf3874431de5805ffa35a553c93707d1863a8888e8aa4)
...
![](./images/onboarding_ethereum_developers_1.png)
...
[this execution chain](https://dashboard.testnet.concordium.com/lookup/7fcad417384d8e36fd2264d16d0ce1385860cdad711d17f7d6c12137c9cbab2e).

![](./images/onboarding_ethereum_developers_2.png)
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="How can I compare/decode the event data that I see on the dashboard?"
  docText={<>
...
![](./images/onboarding_ethereum_developers_11.png)
For example, the above image has an event number tag of ``fd`` (hex encoding) which is ``15*16+13 = 253`` in decimal.
This number tag corresponds to a [burn event](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/concordium-cis2/src/lib.rs#L53)
of a `Cis2` token.
...
[the dashboard](https://dashboard.testnet.concordium.com/lookup) as  0: fe0003532a04.
...
<accordion
  heading="How does the |Chain|_ record CCD transfer events in the test cases?"
  docText={<>
...
transferred as shown in the below [example code](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/examples/recorder/tests/tests.rs#L102).
...
<accordion
  heading="Is there something similar to the SPL Token Program?"
  docText={<>
Yes, please read the [CIS-2 standard](https://proposals.concordium.software/CIS/cis-2.html).
The `CIS-2` standard can represent fungible and non-fungible tokens.
Please explore the corresponding [CIS-2 library](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/concordium-cis2/src/lib.rs).
...
<accordion
  heading="Is there something similar to the Solana's wrapped token wSOL?"
  docText={<>
Yes, please explore:

- the [wCCD tutorial](/docs/mainnet/tutorials/wCCD/index#wCCD)
- the wCCD example [source code](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/examples/cis2-wccd/src/lib.rs).
...
<accordion
  heading="Does Concordium have upgradable smart contracts?"
  docText={<>
Yes, you can choose to make your smart contracts upgradable. See [contract-instance-upgradeability](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instance-upgradeability) for more details.
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="How can I deploy a smart contract to the Concordium chain?"
  docText={<>
You can follow the chapter [deploying a smart contract](/docs/mainnet/tutorials/piggy-bank/deploying#piggy-bank-deploying) in the piggy bank tutorial.
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Is there a smart contract size limit when deploying a contract on-chain?"
  docText={<>
...
<accordion
  heading="What is the `owner` of a smart contract instance on Concordium?"
  docText={<>
...
<accordion
  heading="Can a smart contract deploy/initialize another smart contract on Concordium?"
  docText={<>
...
<accordion
  heading="Can I predict/calculate the address of the smart contract before deploying it?"
  docText={<>
...
<accordion
  heading="Can I invoke another smart contract from within the `init` function?"
  docText={<>
...
<accordion
  heading="Does Concordium have a block explorer?"
  docText={<>
Yes. The official block explorer is [CCDScan](https://testnet.ccdscan.io/).
In addition, you can use the dashboard
to [look up txs](https://dashboard.testnet.concordium.com/lookup),
[explore the network](https://dashboard.testnet.concordium.com),
and observe the [block-producing process](https://dashboard.testnet.concordium.com/chain).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="Can I upload and verify my smart contract code on the block explorer (CCDScan)?"
  docText={<>
CCDScan currently does not support compiling, hosting, or verifying your smart contract code.
You are welcome to publish your smart contract code in public source code management tools such as [GitHub](https://github.com/).
  </>}
  image=""
  fullWidth={true}
/>
...
<accordion
  heading="What is the native currency on Concordium?"
  docText={<>
...
<accordion
  heading="Where do I get some test CCD? Is there a testnet faucet?"
  docText={<>
There are several options to request test CCD:

**Option 1:**
If you just created your account in the Concordium Wallet app, you
...
![](./images/onboarding_ethereum_developers_3.png)
**Option 3:**
Alternatively, you can use [Postman](https://www.postman.com/) and insert the following data before clicking the `Send` button.

![](./images/onboarding_ethereum_developers_4.png)
**Option 4:**
...
<accordion
  heading="Is there something similar to transaction fees?"
  docText={<>
...
<accordion
  heading="What networks can be used for testing?"
  docText={<>
...
<accordion
  heading="Does Concordium have multiSig wallets?"
  docText={<>
Yes. You can create a [multiSig wallet](https://developer.concordium.software/en/mainnet/net/guides/overview-shared-accounts.html) with the desktop wallet.
