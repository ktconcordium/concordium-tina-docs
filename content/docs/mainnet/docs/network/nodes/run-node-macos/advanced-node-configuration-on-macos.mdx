---
title: "Advanced node configuration on macOS"
---
...
3. If the node is running properly with reporting to the network dashboard enabled, you can see the name of the node on the [CCDScan for the mainnet](https://ccdscan.io/) or the [CCDScan for the testnet](https://testnet.ccdscan.io/).
...
[if so configured](/docs/mainnet/docs/network/guides/become-validator#become-a-validator), to produce blocks.
...
1. Remove the environment variables from the configuration file:
...
2. Restart the appropriate node by running the application **Concordium Node Stop [Mainnet/Testnet]** (if running) and then
...
(node-collector-configuration-macos)=
