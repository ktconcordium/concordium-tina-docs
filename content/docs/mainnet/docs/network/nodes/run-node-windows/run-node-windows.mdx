---
title: "Install and manage a node on Windows"
---

(run-node-windows)=


This article explains how to run a Concordium node on Windows on mainnet or testnet, and how to set up your node as a [validator node](/docs/mainnet/docs/network/nodes/run-node-windows/run-node-windows#baker-node-windows).
...
- Meet the [minimum system requirements](/docs/mainnet/docs/network/nodes/node-requirements#node-requirements) for running a node.

(install-upgrade-node-windows)=
...
1. Download the latest `.msi` installer from the [Downloads page](/docs/mainnet/docs/installation/downloads#node-downloads), and run it to launch the setup. If prompted with *Windows protected your PC*, select **More info** and **Run anyway**.

2. Accept the End-User License Agreement and select **Next**; on the following step, specify the **Destination folders**:
   ...
   ![](/images/Node-setup-win-2.png){image} ../images/Node-setup-win-1.png
   :width: 60%
   ```
   ...
   ```{eval-rst}
   <accordion
     heading="Node Runner Service configuration"
     docText={<>
...
To upgrade your Concordium node on Windows, download the latest `.msi` installer from the [Downloads](/docs/mainnet/docs/installation/downloads#node-downloads) page and run it.
...
1. If you have done special configuration of your node, you may want to back up the service config file prior to upgrade, then replace the new one with your backup. This file is typically located at `C:\ProgramData\Concordium\Node Runner\nodes.toml.` (The path on your system is determined by the `Config` value in the registry key `HKEY_LOCAL_MACHINE\SOFTWARE\Concordium\Node Runner`.) Or you can re-enter your values after upgrading. See [Concordium Windows node runner service configuration](/docs/mainnet/docs/network/nodes/run-node-windows/node-runner-service-configuration#node-runner-service-configuration) for information about how to change the service configuration settings.
2. It is a good idea to stop the node before running the installer for the new version so that you do not need to restart your computer. To do this, click on the Start menu and search for Stop Concordium Node Service.

### Uninstall Concordium node on Windows

1. In the **Search** bar, enter *add*, and then select **Add or Remove Programs**.
2. Under **Apps & features**, search for *Concordium*, and then select *Concordium Node*.
3. Select **Modify**, and then in the **Node Setup Wizard** select **Next**.
4. Select **Remove**. If you plan to remove the node permanently, select **Remove node data folder** to remove all files and subfolders in the folder. Don’t select this option if you are only removing the node to perform an upgrade or otherwise want to keep the data, logs, or any other files in the data folder.
5. Select **Next**, and then select **Remove**.
6. If the service is running, when you uninstall it you’ll see a message saying that the system has to restart to update files and services. Select **OK**. (If the service is not running, you’ll not see this message). If you see a message saying *Do you want to allow this app from an unknown publisher to make changes to your device?*, select **Yes**.
7. Select **Finish**, to complete the removal of the Concordium node.
8. To verify that the node has been removed, in the **Search** bar, enter *task manager*, and then select the **Task Manager**.
9. Go to the **Details** tab and verify that node-collector.exe has been removed from the list, and then go to the **Services** tab and verify that ConcordiumNode has been removed from the list.

(baker-node-windows)=

### Run a validator node on Windows

For information about validation on a Windows node, see [import-validator-keys](/docs/mainnet/docs/network/guides/import-validator-keys#import-validator-keys).