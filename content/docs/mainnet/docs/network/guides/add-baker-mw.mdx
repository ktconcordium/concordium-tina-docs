...
Prior to becoming a validator, read [Validator management](/docs/mainnet/docs/network/baker-pool#baker-pool) to learn about best practices for validators. **There is a minimum amount to stake (500000 CCD) to become a validator**.
...
![screen with earning options](/images/cryptoX/cryptox-add-validator1.png)
...
![screen to register validator for first time showing balance, amount to stake, and restake parameters](/images/cryptoX/cryptox-add-validator2.png)
   Choose whether you want to open a [staking pool](/docs/mainnet/docs/resources/glossary#staking-pool) or keep it closed. By opening a staking pool, others can delegate stake to your validator, thus increasing the chance that you are selected to produce a block and earn rewards. If you have a staking pool with delegators, the delegators also earn rewards when you produce blocks. Validators are also paid a commission by the delegators for producing blocks on their behalf. You can choose **Close for delegation** if you do not wish to run a staking pool.

![screen to register validator for first time showing options to open or close pool for delegation](/images/cryptoX/cryptox-add-validator3.png)
...
![screen to register validator for first time showing commission rates for staking pools](/images/cryptoX/cryptox-add-validator4.png)
...
![screen to register validator for first time showing field to paste URL with metadata for staking pools](/images/cryptoX/cryptox-add-validator5.png)
...
![screen to register validator for first time showing all of the validator keys with option to export](/images/cryptoX/cryptox-add-validator6.png)
<Callout
  body={<>
...
![screen confirming validator registration](/images/cryptoX/cryptox-add-valiator7.png)
**Next steps**

8. You need to import your validator keys file to your node in order to start producing blocks. It is preferable to update them on the node as close to the next [pay day](/docs/mainnet/docs/resources/glossary#pay-day) as possible to prevent the node from being down as a validator for a longer time.
...
<Callout
  body={<>
A Single-signature account is an account with only one credential holder. A Multi-signature account is an account where multiple individuals are credential holders and a certain number of credential holders must sign transactions on the account. For more information about multi-signature accounts, see [overview-shared-accounts](/docs/mainnet/docs/guides/overview-shared-accounts#overview-shared-accounts).
  </>}
  variant="info"
/>
<accordion
...
1. Go to **Accounts** and select the account you want to add as validator account and click **More options**.
  </>}
  image=""
  fullWidth={true}
/>
![screen showing accounts with more options highlighted](/images/desktop-wallet/dw-more-options.png)
...
![screen showing amount to stake and restake options](/images/desktop-wallet/dw-add-baker-stake.png)
...
![screen with pool url field and example](/images/desktop-wallet/dw-pool-commission.png)
![screen showing accounts with more options highlighted](/images/desktop-wallet/dw-pool-url.png)
...
<accordion
  ...
  **Generate the transaction**
  ...
  -  [Generate the transaction without signing](#generate-without-sign). This option enables you to export the transaction proposal without signing it. You don't need a LEDGER device but you do need an internet connection.

  -  [Generate and sign the transaction](#generate-sign) This option requires a LEDGER device and an internet connection.
  ...
  *Generate the transaction without signing*
  ...
  #. Select **Generate without signing**. You can now [export the validator credentials](#export-baker-credentials).
...
![](/images/browser-wallet/new/add_validator1.png)
...
![](/images/browser-wallet/new/add_validator2.png)
   Choose whether you want to open a [staking pool](/docs/mainnet/docs/resources/glossary#staking-pool) or keep it closed. By opening a staking pool, others can delegate stake to your validator, thus increasing the chance that you are selected to produce a block and earn rewards. If you have a staking pool with delegators, the delegators also earn rewards when you produce blocks. Validators are also paid a commission by the delegators for producing blocks on their behalf. You can choose *Close for delegation* if you do not wish to run a staking pool.

![screen with balances at the top and text box to enter amount to stake](/images/browser-wallet/new/add_validator3.png)
   On the Commissions screen, set the percentage of rewards you keep when others delegate their stake to your validator pool. Use the sliders to adjust both the [Transaction fee commission](/docs/mainnet/docs/resources/glossary#transaction-fee-commission) and [Block reward commission](/docs/mainnet/docs/resources/glossary#block-reward-commission) - by default, both are set to 100% (meaning you keep all rewards). After setting your desired commission rates, click **Continue** to proceed.

![screen with balances at the top and text box to enter amount to stake](/images/browser-wallet/new/add_validator4.png)
...
![screen with text box to enter url for staking pool information](/images/browser-wallet/new/add_validator5.png)
**Export validator keys and submit**
...
![screen with buttons to show keys or export validator keys](/images/browser-wallet/new/add_validator6.png)
   <Callout
     body={<>
If you're going to transfer the validator keys to someone else, make sure to do so through a secure channel. Generate new keys if you believe the keys have been compromised or lost.
     </>}
...
![](/images/browser-wallet/new/add_validator7a.png)
7. The wallet shows a confirmation screen with a green checkmark indicating that your validator registration transaction has been successfully submitted to the chain. You can see the amount you're validating with. You can click **Transaction details** to view more information about the transaction, or **Return to account** to return to your account overview.

![](/images/browser-wallet/new/add_validator8a.png)
**Next steps**
