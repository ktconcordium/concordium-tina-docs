---
title: "Validation with the Concordium Client"
---
...
- [Run a node](/docs/mainnet/docs/network/nodes/node-requirements#node-requirements)
- [Create an account in a wallet](/docs/mainnet/docs/guides/setup-wallets-lp#setup-wallets-lp)
- Export a file with the account information
- [Install the Concordium Client](/docs/mainnet/docs/installation/downloads#concordium-node-and-client-download)

For general information about validation concepts, see [baker-concept](/docs/mainnet/docs/protocol/concepts-baker#baker-concept).

## Start validation

(import-baker-account)=
...
<Callout
  body={<>
You can only import accounts created in the Concordium Wallet or Concordium Wallet for Web into the Concordium Client. That is, you cannot import accounts created in the Desktop Wallet because they are created using a LEDGER device. You get the account information by exporting a JSON file with the account information.

- To export the private key in Concordium Wallet and Concordium Wallet for Web, see [Export a private key](/docs/mainnet/docs/guides/export-account-private-key#export-key).
  </>}
  variant="info"
/>
...
You can choose an arbitrary name for the `<keys file>`. To register the keys in the network you need to be [running a node](/docs/mainnet/docs/network/nodes/node-requirements#node-requirements) and send a `validator add` transaction to the network:
...
/>
{Note}
To find the range for the commissions, see the Concordium client [Show Chain Parameters command](#consensus show-chain-parameters).
:::
...
- `--no-restake` sets that earnings are not restaked. This flag means that rewards to the staked amount are not added to the validator automatically. Read more about this behavior in the section [Restake earnings](/docs/mainnet/docs/network/guides/become-validator#restake-earnings).
...
If this is a firm requirement for you, then you need to run the [debian package](/docs/mainnet/docs/network/nodes/run-node-ubuntu/run-node-ubuntu#run-node-ubuntu) and configure it appropriately.
  </>}
  variant="warning"
/>
...
To start the node with these validator keys and produce blocks, [configure the node to use the validator keys](/docs/mainnet/docs/network/guides/import-validator-keys#import-validator-keys), and **restart** it. The node will automatically start producing blocks when the validator is included in the validators list for the current epoch.

This change is executed immediately, and it will take effect at the next [pay day](/docs/mainnet/docs/resources/glossary#pay-day) after the one in which the transaction for adding the validator was included in a block. If the change is made in the last epoch before pay day, then the change will not occur until the following pay day.
...
- In the [network dashboard](https://dashboard.mainnet.concordium.software/), the validator ID of the node is shown in the `Validator` column.
...
`--sender` is the name or address of the transaction's sender account. The name is the one that's used when you [import the account](/docs/mainnet/tools/concordium-client#concordium-client-import-accounts-keys) (assuming that this
