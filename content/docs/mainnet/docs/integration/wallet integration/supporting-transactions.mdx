...
A wallet's fundamental purpose is to construct, sign, and broadcast [transactions](/docs/mainnet/docs/resources/glossary#transaction). Each user-facing feature you implement will require your wallet to generate a specific type of transaction payload.
...
This category covers all interactions with [smart contracts](/docs/mainnet/docs/resources/glossary#smart-contract). It enables users to interact with dApps and manage tokens built on the CIS-2 standard (Concordium's universal token standard for Fungible, Non-Fungible, and Semi-Fungible tokens). Common actions include:
...
These transactions are essential for participating in Concordium's [proof-of-stake](/docs/mainnet/docs/resources/glossary#proof-of-stake) network. Key transactions for delegators include:
...
Unique to Concordium's architecture, this category includes the crucial create account transaction. This must be submitted with the cryptographic credential from a user's verified [identity object](/docs/mainnet/docs/resources/glossary#identity-object), linking the new account to that identity.

## Protocol-level tokens

Beyond the standard transaction types, Concordium features [protocol-level tokens (PLTs)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt), a unique class of tokens created directly at protocol level.
...
For further details on PLTs, refer to our [PLT](/docs/mainnet/docs/plt/index#plts) documentation.
