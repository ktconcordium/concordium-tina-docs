...
Also see the Concordium [Whitepaper](https://developer.concordium.software/governance/whitepaper/Concordium%20White%20Paper.pdf) for more details on the terms described below.

:::{glossary}
Account

> An addressable store of funds on the blockchain. An account is associated with one or more *account keys* that can be used to authorize transactions originating from the account, as well as with an [encryption key](/docs/mainnet/docs/resources/glossary#encryption-key). An account is also associated with the account holder's [identity](/docs/mainnet/docs/resources/glossary#identity), although this association is encrypted. This identity can only be disclosed by [privacy guardians](/docs/mainnet/docs/resources/glossary#privacy-guardian-pg), in cooperation with the account's [identity provider](/docs/mainnet/docs/resources/glossary#identity-provider).
...
> A configurable list for a [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) that, if enabled, specifies which accounts are permitted to perform token transfers. If an allow list is supported, both the sender and recipient accounts must be on the list for a token transfer to succeed. This list is administered by the nominated [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account).
...
> User data, such as date of birth or country of residence, that is associated with a user [identity](/docs/mainnet/docs/resources/glossary#identity). Users can choose which attributes should be revealed in each of their accounts.
...
> The basic unit of the blockchain, which is produced by a [validator](/docs/mainnet/docs/resources/glossary#validator). A block contains a (possibly empty) list of [transactions](/docs/mainnet/docs/resources/glossary#transaction), and has a pointer to a previous block (with the exception of the [genesis block](/docs/mainnet/docs/resources/glossary#genesis-block)). A block and its predecessors form a chain, and the sequence of transactions they contain form a ledger. Each block records when it was produced. A block also contains information relating to consensus, for instance establishing which validator created the block, and that the validator was entitled to do so.
...
> Command to take a smart contract module written in Rust and create a Wasm module that can be deployed on chain. The command is run from [cargo-concordium](/docs/mainnet/docs/resources/glossary#cargo-concordium).

Bulletproof

> A type of efficient interactive proof system that enables various [zero-knowledge proofs](/docs/mainnet/docs/resources/glossary#zero-knowledge-proof), including range-proofs, set-membership proofs, and
...
> A [token-governance operation](/docs/mainnet/docs/resources/glossary#token-governance-operation) that destroys a specified amount of tokens from an account, thereby decreasing the total circulating supply of the [PLT](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) and the balance of the originating account. This operation can only be performed by the [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account). For a burn operation to succeed, the token-governance account must have a sufficient balance, the PLT must not be paused, and the PLT's configuration must support burning.
...
> - as a store of value,
> - as a reward for honest behavior (e.g. [validation](/docs/mainnet/docs/resources/glossary#validation)
...
> A sequence of [blocks](/docs/mainnet/docs/resources/glossary#block), starting from the [genesis block](/docs/mainnet/docs/resources/glossary#genesis-block), in which each successive block points to the predecessor. There may be multiple valid chains, and the [consensus](/docs/mainnet/docs/resources/glossary#consensus) protocol establishes which chain is authoritative.

Chain-governance operation

> An operation that can affect the entire chain, and is authorized by the chain governance keys. Chain-governance operations include updating chain parameters, protocol updates, and creating new [PLTs](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt).
...
> To make sure that [zero-knowledge proofs](/docs/mainnet/docs/resources/glossary#zero-knowledge-proof) cannot be reused (e.g., if they are stolen), the verifier can and should specify a challenge string. This can be an arbitrary byte array which is used by the prover (wallet) when producing the proof. The proof will only validate with respect to the challenge that was used to produce it. The verifier server can thus use the challenge to make sure the proofs it is receiving are not reused, and to handle their lifetime (e.g., it can set the challenge it supplied to expire in 5 minutes).
...
> The Concordium command line tool. It is designed as a low-level interface to the Concordium blockchain. It cannot be used to create identities, but it can [import accounts](/docs/mainnet/tools/concordium-client#concordium-client-import-accounts-keys) exported from the Concordium Wallet for Web. Once an account has been imported, Concordium Client can be used to do CCD transfers from the account and other [transaction](/docs/mainnet/docs/protocol/transactions#transactions) types supported by the Concordium blockchain.
...
> The process by which nodes agree which [transactions](/docs/mainnet/docs/resources/glossary#transaction) have occurred and in what order. This consists of [validation](/docs/mainnet/docs/resources/glossary#validation).
...
>
> For delegation in an election, see [Vote delegation](/docs/mainnet/docs/resources/glossary#vote-delegation).

Deny list

> A configurable list for a [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) that, if enabled, specifies which accounts are not permitted to perform token transfers. If a deny list is supported, neither the sender nor the recipient account may be on the list for a token transfer to succeed. This list is administered by the nominated [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account).

Deploy

> Command that takes the built [Wasm](/docs/mainnet/docs/resources/glossary#webassembly) file for a smart contract module and deploys it on chain. This command is run from [Concordium client](/docs/mainnet/docs/resources/glossary#concordium-client).
...
> An [ElGamal] public key associated to an account.
...
> A time period that is approximately one hour on testnet and mainnet. At the start of each epoch, a [leadership election nonce](/docs/mainnet/docs/resources/glossary#leader-election) is computed based on the block nonces of the previous epoch. The leadership election nonce is valid for the duration of the epoch. Each epoch has a nominal ending, and when a block is finalized after this nominal ending then epoch transition occurs.
...
> The first [block](/docs/mainnet/docs/resources/glossary#block) in a [chain](/docs/mainnet/docs/resources/glossary#chain). The genesis block establishes the starting state of the chain, before any transactions have occurred.
...
> Before opening an account on the Concordium platform, one's real-world identity must be verified and recorded by an [identity provider](/docs/mainnet/docs/resources/glossary#identity-provider). A user’s identity is encrypted on-chain, however their real-world identity can be disclosed in response to a valid request from a government authority.
...
> An object issued by the [identity provider](/docs/mainnet/docs/resources/glossary#identity-provider) to the user which allows the user to prove to third parties that their real life identity has been verified by a trusted third party.
...
> An initial account is an account submitted to the chain by the identity provider during the process of requesting a new identity. The initial account can perform all of the same actions as a regular account, however the real-life identity of the initial-account owner is known by the identity provider who submitted it to the chain. In contrast, the real-life identity of the owner of a regular account can only be ascertained by the [Privacy Guardians](/docs/mainnet/docs/resources/glossary#privacy-guardian-pg) in concert with the identity provider.
>
> Initial accounts are only relevant for Desktop Wallet.

Initial supply

> An optional amount of tokens that can be minted to the nominated [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account) when a new [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) is initially created as part of a [chain-governance operation](/docs/mainnet/docs/resources/glossary#chain-governance-operation).
...
> In every round a leader is elected among the validators to produce a new block. The leader is chosen by hashing a leader election nonce and the round number, and interpreting the hash as a random number that picks a validator with probability equal to their relative stake. A new leader election nonce is generated every [epoch](/docs/mainnet/docs/resources/glossary#epoch) by hashing block nonces from the previous epoch. So the sequence of leaders for every epoch is determined at the beginning of the epoch when the leader election nonce is fixed.
>
> The [winning probability](/docs/mainnet/docs/resources/glossary#winning-probability) is proportional to the validator's relative stake.
>
> See [lottery power](/docs/mainnet/docs/resources/glossary#lottery-power).
...
> A validator's lottery power is its relative stake and is therefore proportional to the staked amount of that validator. The lottery power is updated each [pay day](/docs/mainnet/docs/resources/glossary#pay-day), and is based on the stake distribution at the end of the epoch before last. (This delay ensures that the stake distribution is determined before the randomness that fixes the validators for the epoch; otherwise, stakeholders might redistribute their stake to luckier validators, which undermines the security of the system.) [Delegation](/docs/mainnet/docs/resources/glossary#delegator) affects the lottery power of the validator by increasing their stake, thus increasing the odds of that validator being chosen to produce a block.

Mainnet

> The main Concordium network which launched in June 2021. The mainnet will receive periodic upgrades, but in contrast to the [testnet](/docs/mainnet/docs/resources/glossary#testnet), it will never be reset, and accounts created on the mainnet will remain indefinitely.
...
> A proof to determine if an attribute of a user's identity is included in a given set, for example, lives in the EU. Can also be a [non-membership proof](/docs/mainnet/docs/resources/glossary#non-membership-proof).

Mint

> A [token-governance operation](/docs/mainnet/docs/resources/glossary#token-governance-operation) that creates new tokens, increasing both the total circulating supply of the [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) and the balance of the originating account. This operation can only be performed by the [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account), and the newly minted amount is added to this account.
...
> May refer to:
>
> - *Block Nonce*: a randomized value included by the [validator](/docs/mainnet/docs/resources/glossary#validator) in each [block](/docs/mainnet/docs/resources/glossary#block), and used to determine the leadership election nonce.
> - *Leadership Election Nonce*: a randomized value that is updated each [epoch](/docs/mainnet/docs/resources/glossary#epoch) that is used to seed the [leader election](/docs/mainnet/docs/resources/glossary#leader-election) process.
> - [Transaction sequence number](/docs/mainnet/docs/resources/glossary#transaction-sequence-number) (same as account sequence number)
...
> [Token-governance operations](/docs/mainnet/docs/resources/glossary#token-governance-operation) that suspend (pause) or resume (unpause) all balance-changing activities for a [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt), including transfers, minting, and burning. While a PLT is paused, any attempts to perform these operations will fail. These operations can only be performed by the [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account).
...
> A class of efficient interactive [zero-knowledge proof](/docs/mainnet/docs/resources/glossary#zero-knowledge-proof) systems that follow a three-round structure; commitment, challenge, and response. Sigma protocols enable a prover to demonstrate knowledge of secret information without revealing it. They can also be used for OR and AND statements, and can be made non-interactive with the Fiat-Shamir transform.
...
> A test network run by Concordium to test its protocols and software. There can be several test networks in existence at the same time. All the features are tested on the testnet before they are released on the [mainnet](/docs/mainnet/docs/resources/glossary#mainnet).
...
> A specific Concordium account nominated during the creation of a [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) that is granted the capabilities to perform administrative operations associated with that PLT. As of protocol version 9, this single account holds full authority over all [token-governance operations](/docs/mainnet/docs/resources/glossary#token-governance-operation), including minting, burning, administering allow/deny lists, and pausing/unpausing the token.

Token-governance operation

> Administrative operations that control the behavior and supply of a [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt). These operations can only be performed by the [token-governance account](/docs/mainnet/docs/resources/glossary#token-governance-account) for that specific PLT. Examples include minting tokens, burning tokens, administering allow- and deny-lists, and pausing or unpausing the token. These operations are submitted as part of a token update transaction.

Token-holder operation

> Operations that individual token holders can perform with their [PLTs](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt). As of protocol version 9, the only token-holder operation is the token transfer.

Token ID

> A unique identifier assigned to each [Protocol-Level Token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) when it is created via a [chain-governance operation](/docs/mainnet/docs/resources/glossary#chain-governance-operation). This identifier is used to uniquely identify the token on the Concordium chain in transactions and through its gRPC interface. A TokenId can be up to 128 characters long, consisting of a-z, A-Z, 0-9, -, ., and %, and is matched in a case-insensitive manner.

Token Kernel

> The Token Kernel is a low-level component of the Concordium protocol that provides fundamental state update operations for [Protocol-Level Tokens (PLTs)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt). It defines the basic functionalities that the [Token Module](/docs/mainnet/docs/resources/glossary#token-module) uses to implement more complex transactions and queries. The Token Kernel ensures core invariants are maintained, such as that all token transfers are accompanied by an event that logs the transfer.

Token Module

: The component within the Concordium protocol responsible for the general handling and management of [Protocol-Level Tokens (PLTs)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt). Protocol version 9 supports a single Token Module, but future versions may introduce new Token Modules with additional functionality. When the PLT is created, the Token Module associated with it is identified by a reference consisting of a SHA-256 hash.

Token transfer

> The operation to send a specified amount of a [Protocol-level token (PLT)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt) from one Concordium account to another.

Token update transaction

> A new transaction type introduced in Protocol Version 9 to facilitate operations for [Protocol-Level Tokens (PLTs)](/docs/mainnet/docs/resources/glossary#protocol-level-token-plt). This transaction identifies a specific PLT by its [Token ID](/docs/mainnet/docs/resources/glossary#token-id) and contains a list of token operations that can include both [token-governance operations](/docs/mainnet/docs/resources/glossary#token-governance-operation) (e.g., mint, burn, pause) and [token-holder operations](/docs/mainnet/docs/resources/glossary#token-holder-operation) (e.g., transfer). Operations within the transaction are executed in order, and if any single operation fails, the entire transaction is rejected and has no effect.

Total effective stake

> The total amount of stake in staking pools excluding passive delegation and any amount that exceeds the [staking pool bounding caps](/docs/mainnet/docs/protocol/concepts-delegation#delegation-caps).

Transaction

> An atomic operation that defines a change of state in the ledger, such as transferring funds from one account to another. A transaction typically has a sender account and a [transaction sequence number](/docs/mainnet/docs/resources/glossary#transaction-sequence-number), and incurs a fee. The sender account must sign the transaction to authorize it. (The exception to this is a credential deployment transaction that creates a new account, which does not have a sender account.)
...
> The process of production of [blocks](/docs/mainnet/docs/resources/glossary#block).

Validator

> A node that participates in the production of [blocks](/docs/mainnet/docs/resources/glossary#block), referred to as *validation*.
...
[ciphertexts]: https://en.wikipedia.org/wiki/Ciphertext
