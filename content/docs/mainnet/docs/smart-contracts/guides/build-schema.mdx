...
[setup-env](/docs/shared/setup-env#setup-env) will help you.
...
trait. This is already done for all base types and some other types (see [list of types implementing the SchemaType]).
...
For example, the [Cis2 library](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/concordium-cis2/src/lib.rs)
...
<Callout
  body={<>
Event schemas can be represented by the `TaggedEnum Type` ([see list of types available to implement a schema](https://docs.rs/concordium-contracts-common/latest/concordium_contracts_common/schema/enum.Type.html)).
...
[here](/docs/mainnet/docs/smart-contracts/contract-schema#contract-schema-which-to-choose).
:::
...
output in base64 format that is currently supported in the Concordium Wallet for Web. When building
...
- The command `schema-base64` and the flag `--schema-base64-out` convert the schema into base64 format. This is the format that is currently used by the Concordium Wallet for Web and most of our frontend examples.
...
[cargo-concordium]: https://crates.io/crates/cargo-concordium
[list of types implementing the schematype]: https://docs.rs/concordium-contracts-common/latest/concordium_contracts_common/schema/trait.SchemaType.html#foreign-impls