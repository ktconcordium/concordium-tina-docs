...
return value when rejecting](https://docs.rs/concordium-std/latest/concordium_std/#signalling-errors). This is also the case when using a custom error type.
...
You can derive the implementation automatically with [#[derive(Reject)]](https://docs.rs/concordium-std/latest/concordium_std/derive.Reject.html) if
...
<Callout
  body={<>
Adding [schemas](/docs/mainnet/docs/smart-contracts/guides/build-schema#build-schema) to your errors makes them more useful in
...
The error codes are also listed in [the library documentation on docs.rs](https://docs.rs/concordium-std/latest/concordium_std/#signalling-errors).
