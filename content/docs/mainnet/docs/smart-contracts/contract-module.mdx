...
among the contracts and can optionally contain [contract schemas](/docs/mainnet/docs/smart-contracts/contract-schema#contract-schema) and verification data.
...
:::{seealso}
Check out [host-functions](/docs/mainnet/docs/smart-contracts/references/host-fns#host-functions) for a complete reference.
:::

## On-chain language

On the Concordium blockchain the smart contract language is a subset of [Web
Assembly][web assembly] (Wasm in short) which is designed to be a portable compilation
...
You can read [this article](/docs/mainnet/docs/smart-contracts/develop-contracts#writing-smart-contracts) for more information regarding using Rust on Concordium.
...
[Deploying](/docs/mainnet/docs/smart-contracts/guides/deploy-module#deploy-module) a module to the chain means submitting the module bytecode as a
...
smart contract. To execute, a user must first create an *instance* of a contract. See [contract-instances](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instances) for more information.
...
module. The concrete mechanism used for this is the [Web Assembly] export
...
contracts, you associate the functions using a naming scheme:
