...
- Be ready if things go wrong after deployment:
  ...
  - provide “pause” functionality (see more in the [best-practices-code-structure](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-code-structure) section of this document);
  - implement additional approvals/wait periods for dangerous operations;
  - make your contract [upgradable](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instance-upgradeability);
...
See [Introduction to smart contracts](/docs/mainnet/docs/smart-contracts/introduction#introduction) for basic information.
...
- Provide the pause functionality: add a boolean flag to the state controlling whether the contract is active.
  ...
  See a code snippet from the [wCCD contract example](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/examples/cis2-wccd/src/lib.rs) below.
...
(best-practices-external-call-failure)=
...
In the [auction contract](https://github.com/Concordium/concordium-rust-smart-contracts/blob/main/examples/auction/src/lib.rs) example it is safe to refund the previous bidder as part of the bidding functionality, because on Concordium transferring to accounts is guaranteed not to execute any code.
...
See [contract-instance-operations](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instance-operations) for more information.
...
For example, if you are interested in historical data for all the bids, but your contract logic requires only the highest bid to make a decision, consider [logging events](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instances-logging-events) instead.
...
Calls to your contract might change its state through entrypoints that permit updating the state; see the [best-practices-reentrancy](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-reentrancy) section of this document.
...
- *Avoid complex interactions*.
  ...
  For example, instead of using the *proxy pattern for upgradability*, use [natively upgradable contracts](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instance-upgradeability).
  Using the proxy makes the implementation more complex by introducing contract interactions.
  Proxies can be useful for other purposes, but for upgradability, it is recommended to use [natively upgradable contracts](/docs/mainnet/docs/smart-contracts/contract-instances#contract-instance-upgradeability).
  ...
  The address of the main contract can be updated in the state of the proxy contract, making the whole setup upgradable. (See [here](https://docs.openzeppelin.com/contracts/4.x/api/proxy) for more information).
  :::

- *Think about the contract state*.
  Do not assume that the contract state stays the same after an external call.
  See the [best-practices-reentrancy](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-reentrancy) section of this document for details.
...
- *Protect from denial-of-service (DoS) attacks*.
  ...
  Read more in the [best-practices-external-call-failure](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-external-call-failure) section of this document.

(best-practices-reentrancy)=
...
See an [example](/docs/mainnet/docs/smart-contracts/guides/unit-test-contract#reentracny-unit-testing) based on [the DAO](<https://en.wikipedia.org/wiki/The_DAO_(organization)>) Ethereum smart contract vulnerability of how reentrancy can be discovered using unit testing.
...
- If you need to perform some state changes after an external call use [invoke_contract_read_only](https://docs.rs/concordium-std/latest/concordium_std/struct.ExternHost.html#method.invoke_contract_read_only).
...
(best-practices-code-automated-testing)=

### Automated testing

The Concordium standard library [concordium-std] offers several possibilities for testing the smart contract code.

- Use [Integration testing](/docs/mainnet/docs/smart-contracts/guides/integration-test-contract#integration-test-contract) to test particular cases where you define what is the valid output.
- (**Deprecated**) Use [Unit testing](/docs/mainnet/docs/smart-contracts/guides/unit-test-contract#unit-test-contract) to test particular cases where you define what is the valid output.
- (**Deprecated**) [Property-based testing](/docs/mainnet/docs/smart-contracts/guides/unit-test-contract#writing_property_based_tests) is a variant of randomized testing that repeatedly checks a *property* with randomly generated input.

Use the [smart contract specification](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-specification) guidelines from this document to come up with cases and properties to test.
...
- your code follows the [recommended structure](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-code-structure) described in this document;
- you looked carefully for all *known* sources of issues, e.g. [external calls](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-external-calls), arithmetic overflows, etc.
- you have a *disaster recovery plan*: the pause functionality, upgradability, etc.
- you used formatting and linting tools (see the [Contributing section](https://github.com/Concordium/concordium-rust-smart-contracts#contributing));
- you [documented your code properly](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-code-documentation);
- you tested your code according to the specification, using both [automated](/docs/mainnet/docs/smart-contracts/best-practices/development#best-practices-code-automated-testing) and manual testing;
