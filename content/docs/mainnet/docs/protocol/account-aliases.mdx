...
To show aliases, [run a transaction in Concordium Client](/docs/mainnet/tools/transactions#account-aliases).
...
- `Rust SDK`: use the `get_alias` and `is_alias` methods on the [AccountAddress type](https://docs.rs/concordium-rust-sdk/latest/concordium_rust_sdk/common/types/struct.AccountAddress.html#method.get_alias). An [utility](https://github.com/Concordium/concordium-rust-sdk/blob/main/examples/aliases.rs) that checks whether a list of accounts are all aliases of each other is also available.
- `TypeScript SDK`: use the `getAlias` and `isAlias` [methods](https://github.com/Concordium/concordium-node-sdk-js/blob/main/packages/sdk/src/types/AccountAddress.ts#L182) for reference.
...
<Callout
  body={<>
Token Behavior with Alias Account Addresses
