---
title: "Identity framework on Concordium"
---
...
Concordium features two independent but interrelated identity layers that work together to provide a complete identity solution, [Concordium ID](/docs/mainnet/docs/protocol/identity#concordium-id) and [Web3 ID](/docs/mainnet/docs/protocol/identity#web3-id).

(concordium-id)=

## Concordium ID

The Concordium ID (also known as Base ID or Core ID) is the protocol-level identity system on Concordium. All users must obtain a Concordium ID through identity verification with an approved [Identity Provider](/docs/mainnet/docs/resources/glossary#identity-provider) before they can create and use accounts on the blockchain.
...
- {doc}`Conduct transactions <transactions>` within the networkusing [zero-knowledge proofs](/docs/mainnet/docs/resources/glossary#zero-knowledge-proof) without revealing the underlying data
...
For more information, see [Company identity creation](https://developer.concordium.software/en/mainnet/net/guides/company-identities.html#company-identities).
  </>}
  variant="info"
/>
...
The Web3 ID suite (also referred to as Portable ID) is a system for issuing and managing [verifiable credentials](/docs/mainnet/docs/resources/glossary#verifiable-credential), based on the
...
- Portable [KYC](/docs/mainnet/docs/resources/glossary#kyc)
...
- **Account Holder Identity Records**: A collection of records related to the account owner, including their identity information and an encrypted [linking key](/docs/mainnet/docs/resources/glossary#linking-key) that links to any other accounts opened with the same identity document. This data is stored in the IDP’s database.
