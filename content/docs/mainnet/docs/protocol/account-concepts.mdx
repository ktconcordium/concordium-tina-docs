...
This section covers important technical aspects of how accounts work on the Concordium blockchain, including [account balances](/docs/mainnet/docs/protocol/account-concepts#managing-account-balances), [initial accounts](/docs/mainnet/docs/protocol/account-concepts#initial-accounts), and [account-sequence-numbers](/docs/mainnet/docs/protocol/account-concepts#account-sequence-number).
...
- some of the public balance is locked up because it was received via a
  [transfer with schedule](/docs/mainnet/docs/resources/glossary#transfer-with-schedule)
...
Then on January 2 the account receives 50 CCD via a [transfer with
schedule](/docs/mainnet/docs/resources/glossary#transfer-with-schedule) with the release scheduled for
...
When using Concordium Wallet or Concordium Wallet for Web, you create all accounts yourself. The procedure is different when using Desktop Wallet.

When using Desktop Wallet, you get an initial account at the same time as an [identity](/docs/mainnet/docs/protocol/identity#reference-identity) has been issued by an [identity provider](/docs/mainnet/docs/resources/glossary#identity-provider). As the initial account is submitted to the chain by the identity provider, the identity provider knows the owner of the initial account. For this reason, you may not want to use the initial account and create a regular account instead. There can only be one initial account for one identity.

You additionally [create account keys](/docs/mainnet/docs/guides/recovery#backup-import-recover) for an initial account, which you store privately. The identity provider then verifies the validity of your identity information
...
Based on your user identity certificate the you can subsequently create other accounts (see below) that can only be linked to you if the [privacy guardians](/docs/mainnet/docs/resources/glossary#privacy-guardian-pg) and the identity provider are
involved. This gives you a way to create accounts with an additional layer of privacy protection compared to that in the initial account. The owner of a regular account is not known to the identity providers or any other single entity. To facilitate compliance with relevant regulations, a regular account can only be created from an *identity* which is issued [off-chain](/docs/mainnet/docs/resources/glossary#off-chain) by an Identity provider. While an account has to be created from an identity, your privacy is still protected, and the account owner's identity can only be revealed via the process of [disclosing an identity](/docs/mainnet/docs/protocol/identity-disclosure-processes#reference-identity-disclosure-processes), which can only happen under stringent regulations. In particular, a key feature of the design of identities and accounts is that the identity provider cannot reveal the identity of an account on their own.
...
You can [look up the sequence number](/docs/mainnet/tools/transactions#account-seqno) from an up to date node using Concordium Client.

The Concordium Wallet keeps track of the sequence number and assigns the correct one when sending transactions.
