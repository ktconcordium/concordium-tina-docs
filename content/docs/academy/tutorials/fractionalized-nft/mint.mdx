...
Remember that your fractionalization logic starts with collateralizing the CIS-2 NFT. In order to mint a CIS-2 NFT you can follow the [Mint a Semi-fungible token tutorial](#sft-index). As previously mentioned in this tutorial, you will use cis2-multi smart contract for minting our token and you can find the full code here. The screenshot below shows only the `mint()` function. You can find build/deploy processes in the tutorial.
